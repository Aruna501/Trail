...
<div className='tour-container'>
    <h2 className='tour-head'>Our Tours
    ...
    {tour.map(data=>(
        ...
        <div className='tour-info'>
            <div className='tour-card'>
                        <img src={data.image} alt={data.name} width="100px" height="100px" />
            <p className='price-card'>${data.price}</p>
            </div>
            <div className='info-cond'>
            <h5>{data.name}</h5>
                            <p className='info-card'>{showmore ? data.info : `${data.info.substring(0, 250)}...`}
                                           <button className='btn-sm' onClick={()=>setShowmore(!showmore)}>
